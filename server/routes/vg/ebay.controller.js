    ...
    axios
      ...
      .then((result) => {
        ...
        if (check) {
          ...
          const link = `https://www.ebay.com/sch/i.html?_nkw=${searchTerms}`;
          res.status(200).json({
            error: false,
            payload: { highest, lowest, average, results: pLen, link }
          });
        } else {
          res.status(204).json({
            error: true,
            message: 'ERROR: Ebay price results fetched; error while parsing results'
          });
        }
      })
      .catch((err) => {
        ...
        res
          .status(500)
          .json({ error: true, message: 'ERROR: Problem fetching eBay price results.', code: err });
      });
  } else {
    res.status(400).json({
      error: true,
      message: "ERROR: Request malformed/doesn't contain required arguments."
    });
  }
};
